...
async def check_order_read_access(order_id: int, user: Annotated[User, Depends(get_user_by_access_token)],
                                  db_factory: Annotated[Tuple[Queries, Connection], Depends(queries)]) -> None:
    db, conn = db_factory
...
async def check_all_client_orders_read_access(user_id: int,
                                              user: Annotated[User, Depends(get_user_by_access_token)]) -> None:
    if not (user_id == user.id or user.is_employee):
        raise PermissionOwnerError


async def check_order_delete_access(order_id: int, user: Annotated[User, Depends(get_user_by_access_token)],
    ...
    raise PermissionOwnerError('Orders could be deleted either by cleanix managers or order owners.')


async def check_order_modify_access(order_id: int, db_factory: Annotated[Tuple[Queries, Connection], Depends(queries)],
...
async def check_order_mark_in_process_access(employee: Annotated[Employee, Depends(get_worker)]):
    return employee
